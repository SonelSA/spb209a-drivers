...
#ifdef HDW_ENABLE_CCA_ED

mlan_status hdw_configure_cca_ed_enabled(pmlan_private pmpriv)
{
   ...
   typedef struct
   {
      struct _HostCmd_DS_GEN hdr;
      ed_ctrl_t              _2g;
      ed_ctrl_t              _5g;
   }host_misc_cmd_ed_mac_ctrl_conf_v2_pdu_t;

   /*
   This frame must match generic format of mwifiex_ds_misc_cmd
   */
   typedef struct
   {
      t_u32                                   pdu_size;
      host_misc_cmd_ed_mac_ctrl_conf_v2_pdu_t pdu;
   }host_misc_cmd_ed_mac_enable_frame_t;

   mlan_status ret;
   host_misc_cmd_ed_mac_enable_frame_t cmd_frame;

	ENTER();

   memset(pmpriv->adapter,&cmd_frame,0x00,sizeof(cmd_frame));

   /*
   See "AN-12104 Application Note on Testing to EN 300 328 Requirements Rev 0.7", Page 10
   */
   cmd_frame.pdu_size           = sizeof(host_misc_cmd_ed_mac_ctrl_conf_v2_pdu_t);
   cmd_frame.pdu.hdr.size       = sizeof(host_misc_cmd_ed_mac_ctrl_conf_v2_pdu_t);
   cmd_frame.pdu.hdr.command    = 0x130;
   cmd_frame.pdu._2g.enable     = 0x01;
   cmd_frame.pdu._2g.offset     = 0;
   cmd_frame.pdu._2g.on_period  = 0x14;
   cmd_frame.pdu._2g.off_period = 0;
   cmd_frame.pdu._2g.bitmap     = 0x01;

   cmd_frame.pdu._5g.enable     = 0x01;
   cmd_frame.pdu._5g.offset     = 0;
   cmd_frame.pdu._5g.on_period  = 0x14;
   cmd_frame.pdu._5g.off_period = 0;
   cmd_frame.pdu._5g.bitmap     = 0x01;

   ret =  wlan_prepare_cmd(pmpriv,
                           0 /* 0 implies Host cmd */,
                           0,
                           0,
                           MNULL,
                           &cmd_frame);
...
mlan_status hdw_force_cca_ed_now(pmlan_private pmpriv)
{
   mlan_status res;
   struct _mlan_ds_reg_rw cca_reg_rdwr;

	ENTER();

   for(;;) {
      /* Enable CCA Energy Detect */
      cca_reg_rdwr.offset = 0x70;
      cca_reg_rdwr.value  = 100;
      res = wlan_prepare_cmd(pmpriv,
                             HostCmd_CMD_BBP_REG_ACCESS,
                             HostCmd_ACT_GEN_SET,
                             0,
                             MNULL,
                             &cca_reg_rdwr);
      if (res != MLAN_STATUS_SUCCESS)
         break;

      cca_reg_rdwr.offset = 0x5f;
      cca_reg_rdwr.value  = 68;
      res = wlan_prepare_cmd(pmpriv,
                             HostCmd_CMD_BBP_REG_ACCESS,
                             HostCmd_ACT_GEN_SET,
                             0,
      ...
      cca_reg_rdwr.offset = 0x60;
      cca_reg_rdwr.value  = 73;
      res = wlan_prepare_cmd(pmpriv,
                             HostCmd_CMD_BBP_REG_ACCESS,
                             HostCmd_ACT_GEN_SET,
                             0,
...
mlan_status hdw_enable_cca_ed(pmlan_private pmpriv)
{
	ENTER();
   PRINTM(MMSG, "HDW Enable CCA ED\n");
   //   return hdw_force_cca_ed_now(pmpriv);
   LEAVE();
   return hdw_configure_cca_ed_enabled(pmpriv);
}
