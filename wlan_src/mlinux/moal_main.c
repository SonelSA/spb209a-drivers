...
/** Driver version */
char driver_version[] =
	"SD8887-%s-C" KERN_VERSION "15" MLAN_RELEASE_VERSION
	"-GPL" "-(" "FP" FPNUM ")"
#ifdef	DEBUG_LEVEL2
	"-dbg"
#endif
...
#define MAC_SLEW_RATE_REG           0x80002328
#define MAC_SLEW_RATE_PAD_OFFSET    12
...
woal_set_sdio_slew_rate(moal_handle *handle)
{
	...
	if (!priv)
		return MLAN_STATUS_FAILURE;
	mac_reg = MAC_SLEW_RATE_REG;
	bit_offset = MAC_SLEW_RATE_PAD_OFFSET;
			...
			char *country_txpwrlimit)
{
	...
	if (type == INIT_CFG_DATA) {
		...
		if (req_fw_nowait && !handle->fw_reload) {
			...
			if (status == -ENOENT) {
#ifdef HDW_CLUSTER_COUNTRY_REGION
...
woal_dump_priv_drv_info(moal_handle *handle, t_u8 *buf)
{
	...
	ENTER();
	if (!handle || !buf) {
		PRINTM(MERROR, "%s: can't retreive info\n", __func__);
		LEAVE();
		return 0;
	}
...
#define DEBUG_DUMP_CTRL_REG               0xA2
#define DEBUG_DUMP_START_REG              0xA3
#define DEBUG_DUMP_END_REG                0xAA
...
woal_dump_firmware_info_v2(moal_handle *phandle)
{
	...
	if (ret) {
		PRINTM(MERROR, "SDIO READ MEM NUM ERR\n");
		goto done;
	}
...
#ifdef HDW_CLUSTER_COUNTRY_REGION
		table_load_result = woal_set_user_init_data(handle, COUNTRY_POWER_TABLE, MOAL_IOCTL_WAIT, file_path);
		if (MLAN_STATUS_SUCCESS == table_load_result) {
			PRINTM(MINFO, "Download power table for %c%c to firmware success\n",(strstr(country_name, "_")+1)[0],(strstr(country_name, "_")+1)[1]);
			break;
		} else if (MLAN_STATUS_RESOURCE == table_load_result) {
				if (MLAN_STATUS_SUCCESS != get_cluster_region(strstr(country_name, "_")+1, strstr(country_name, "_")+1)) {
					PRINTM(MFATAL, "No cluster region for %c%c.\n",(strstr(country_name, "_")+1)[0],(strstr(country_name, "_")+1)[1]);
					break;
				}
				PRINTM(MINFO, "Found cluster region %c%c.\n",(strstr(country_name, "_")+1)[0],(strstr(country_name, "_")+1)[1]);
		} else {
...
#define FW_RESET_REG  0x0B6
#define FW_RESET_VAL  1
...
woal_init_module(void)
{
	...
	if (ret == MLAN_STATUS_SUCCESS)
		...
		PRINTM(MERROR, "wlan: Driver loading failed\n");
