...
/**
 *  @brief This function checks if a country code is within a cluster region (eg GB is within EU). Currently EU is the only supported cluster region.
 *
 *  @param region_string    Region (country) string
 *
 *  @return                 MLAN_STATUS_SUCCESS and cluster region code written when within cluster region, otherwise MLAN_STATUS_FAILURE
 */
mlan_status
get_cluster_region(char *region_string, char * cluster_region_string)
{
	t_u8 i;
	char eu[] = "EU";
	mlan_status rc = MLAN_STATUS_FAILURE;

	ENTER();
   
	for(;;) {
		if (!memcmp(region_string, eu,
				COUNTRY_CODE_LEN - 1)) {
			/* EU is not within EU... */
			break;
		}
		...
		for (i = 0; i < ARRAY_SIZE(eu_country_code_table); i++) {
			if (!memcmp(region_string, eu_country_code_table[i],
				    COUNTRY_CODE_LEN - 1)) {
				PRINTM(MIOCTL, "found cluster region in EU table for %c%c\n", region_string[0], region_string[1]);
				rc = MLAN_STATUS_SUCCESS;
				memcpy(cluster_region_string, eu, COUNTRY_CODE_LEN - 1);
				break;
			}
		}
		break;
